...
2. **Garage** — pick chassis (BRUTE / VECTOR / HALO), circuit (SUNSET OVAL / NEON EIGHT / RIDGE RUN / MIDNIGHT SEED), lap count (1–9, default 3).
3. **Race** — countdown 3-2-1-GO, drift bank accumulates across all laps, time bonus at the end.
4. **Results** — 1-3 stars based on final score, breakdown of bank / time / style / clean / wall penalties. Career rank auto-updates (D → C → B → A → S).
5. **Pause** — Esc or **P** during a race. Resume / Restart / Garage. Also auto-fires on window blur.
6. **Track editor** — see below.
7. **Tune panel** — see below.
